class Solution {
    ...
    public void bfs(char[][] grid, Set<Pair<Integer, Integer>> visited, int r, int c) {
        ...
        while(!queue.isEmpty()) {
            ...
            for(Pair<Integer, Integer> dir : directions) {
                ...
                if (rd >= 0 && rd < grid.length && rc >= 0 && rc < grid[0].length && grid[rd][rc] == '1' && !visited.contains(new Pair<>(rd, rc))) {
                    visited.add(new Pair<>(rd, rc));
